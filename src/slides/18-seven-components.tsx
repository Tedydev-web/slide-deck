...
import { Settings, User, BookOpen, Database, Wrench, MessageCircle, Lock, Terminal } from 'lucide-react';
import { theme, gradients } from '../lib/theme';

export default function Slide18SevenComponents() {
  /*
   * 8 parts of context — aligned with Phil Schmid's Context Engineering
   * framework. "Định nghĩa tool" (tool schemas) is structurally distinct
   * from "Kết quả tool" (tool output) and must be in context before the
   * LLM can call any function.
   */
  const components = [
    ...
    { icon: Terminal, label: 'Định nghĩa tool', labelKr: '도구 정의', desc: 'Schema & mô tả function', descKr: '스키마 및 함수 설명' },
  ...
  return (
    <SlideLayout background={gradients.editorial}>
      <div style={{ width: '100%', height: '100%', display: 'flex', flexDirection: 'column', justifyContent: 'center', padding: '50px 70px' }}>
        ...
        >
<span style={{ color: theme.colors.accent }}>8</span> thành phần của Context
        </motion.h2>
        <motion.div
